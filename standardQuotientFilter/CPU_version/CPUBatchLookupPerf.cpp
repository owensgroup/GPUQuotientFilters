...
int main(int argc, char* argv[])
{
    ...
    double inserttRate = (double) numValues / (timespec2usec(endTime) - timespec2usec(startTime));

    //Random lookups at batch size
    unsigned int *newRandomValues = new unsigned int[batchSize];
    generateRandomNumbers(newRandomValues, batchSize);
    clock_gettime(CLOCK_MONOTONIC, &startTime);
    for(int i = 0; i < batchSize; i++){
        int slotValue = mayContain(&qfilterPerfTest, newRandomValues[i]);
    }
    clock_gettime(CLOCK_MONOTONIC, &endTime);
    double randomLookupRate = (double) batchSize / (timespec2usec(endTime) - timespec2usec(startTime));
    printf("%f\n", randomLookupRate);

    delete[] randomValues;
    delete[] newRandomValues;
