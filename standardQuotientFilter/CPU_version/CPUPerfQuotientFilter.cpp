...
//  printf("%f\t", successfulLookupRate);

    //Random lookups
    unsigned int *newRandomValues = new unsigned int[numValues];
    generateRandomNumbers(newRandomValues, numValues);
    clock_gettime(CLOCK_MONOTONIC, &startTime);
    for(int i = 0; i < numValues; i++){
        int slotValue = mayContain(&qfilterPerfTest, newRandomValues[i]);
    }
    clock_gettime(CLOCK_MONOTONIC, &endTime);
    double randomLookupRate = (double) numValues / (timespec2usec(endTime) - timespec2usec(startTime));
    printf("Random lookup rate = %f million ops/sec\n", randomLookupRate);
//  printf("%f\t", randomLookupRate);
