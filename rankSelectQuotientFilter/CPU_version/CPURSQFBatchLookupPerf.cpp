...
int main(int argc, char* argv[])
{
    ...
    for(int i = 0; i < batchSize; i++){
        int slotValue = lookup(&test_cqf, batchValues[i]);
    }
    clock_gettime(CLOCK_MONOTONIC, &endTime);
    double batchLookupRate = (double) batchSize / (timespec2usec(endTime) - timespec2usec(startTime));
    printf("%f\n", batchLookupRate);
